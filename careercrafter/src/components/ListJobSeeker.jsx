...
export const ListJobSeeker = () => {
    ...
    const deleteJobSeekerById = (id) => {
        ...
        JobSeekerService.deleteJobSeekerById(id)
            .then((response) => {
                console.log("response recieved from saved API..." + JSON.stringify(response.data))
                fetchAllJobSeekers();
            }).catch(error => { console.log("error recieved from saved API...", error) });
    }
    useEffect(()=>{
                ...
                <tbody>
                    {
                        jobSeekerArray.map((jobSeeker, key) => <tr key={key}>
                            ...
                            <td><Link to={`/jobSeeker/update/${jobSeeker.jobSeekerId}`} className='btn btn-success'>Update</Link>
                                <br />
