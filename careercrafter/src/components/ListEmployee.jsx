...
export const ListEmployee = () => {
    ...
    const deleteEmployerById=(id)=>{
        ...
        EmployerService.deleteEmployerById(id).then((response)=>{
            console.log("response recieved from the API in the List Employer component...",JSON.stringify(response.data),"id",id)
            fetchAllEmployer();
        }).catch(error=>{console.log("Error recieved in the list employer component in delete employer ",error,"  id ",id)})
    }
    ...
    <div className="container">
        ...
        <Link to="/addEmployer" className = "btn btn-primary mb-3">Add Employer</Link>
            ...
            <tbody>
                {
                    employeeArray.map((employee,key)=><tr key={key}>
                        ...
                        <td><Link to={`/employee/update/${employee.employerId}`} className='btn btn-success'>update</Link>
                        <br/> <button className="btn btn-danger" onClick={()=>deleteEmployerById(employee.employerId)}>Delete</button>
                       </td>
                    </tr>
                    )
                }
